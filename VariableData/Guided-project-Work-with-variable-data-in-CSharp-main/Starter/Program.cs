  ...
  switch (menuSelection)
  {
    ...
    case "2":
      ...
      do
      {
        ...
        noMatchesDog = true;
        Console.WriteLine($"\ndesired characteristic: {dogCharacteristic}");
        ...
        for (int i = 0; i < maxPets; i++)
        {
          if (ourAnimals[i, 1].Contains("dog"))
          {
            ...
            var dogDescription = new StringBuilder();
            dogDescription.AppendLine(ourAnimals[i, 4]);
            dogDescription.AppendLine($"\n{ourAnimals[i, 5]}");

            if (dogDescription.ToString().Contains(dogCharacteristic))
            {
              Console.WriteLine($"\nOur dog {ourAnimals[i, 0]} is a match!");
              Console.WriteLine(dogDescription.ToString());
              noMatchesDog = false;
            }
          }
        }
        ...
        if (noMatchesDog)
          Console.WriteLine($"None of our dogs are a match found for: {dogCharacteristic.ToString()}\n");

        Console.WriteLine("Wanna try another desired dog characteristic to try for (y/n)");
        readResult = Console.ReadLine()?.ToLower();
        continueSearch = (readResult == "y");
