...
do
{
  ...
  switch (menuSelection)
  {
    ...
    case "2":
      ...
      string[] dogCharacterArray = Array.Empty<string>();

      while (dogCharacteristics == "")
      {
        // #2 have user enter multiple comma separated characteristics to search for
        Console.WriteLine($"\r\nEnter one or more desired dog characteristics seperated by commas");
        readResult = Console.ReadLine();
        if (readResult != null)
        {
          dogCharacteristics = readResult.ToLower();
          dogCharacterArray = dogCharacteristics.Split(',').Select(s => s.Trim()).ToArray();
          Array.Sort(dogCharacterArray);
          Console.WriteLine();
        }
      }
      ...
      for (int i = 0; i < maxPets; i++)
      {
        if (ourAnimals[i, 1].Contains("dog"))
        {
          ...
          foreach (string characteristic in dogCharacterArray)
          {
            // #5 update "searching" message to show countdown
            foreach (int countDown in countDownNumbers)
            {
              foreach (string icon in searchingIcons)
              {
                Console.Write($"\rsearching...{characteristic} {icon} {countDown}");
                Thread.Sleep(250);
              }

              // Clear the line after each search animation cycle
